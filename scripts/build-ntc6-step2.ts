...
type BlockSpec = { kind: "heading" | "paragraph" | "list-item" | "formula-ref" | "table-ref"; page: number; from: number; to?: number; norm?: string; asset?: string; refs?: Ref[]; listMarker?: "bullet" | "dash" | "none"; listLevel?: number };
...
const cell = (text: string, latex?: string, extra: Record<string, any> = {}): any => ({ text, ...(latex ? { latex } : {}), ...extra });
const tables = [
    { id: t("6.4.vi"), unit: "6.4.3.1.2", number: "6.4.VI", page: 197, caption: "Coefficiente parziale γT per le verifiche agli stati limite ultimi di pali soggetti a carichi trasversali", columnCount: 1, columnWidths: [100], headers: [[cell("Coefficiente parziale (R3)", "\\text{Coefficiente parziale }(\\mathrm{R3})")]], rows: [[cell("γT = 1,3", "\\gamma_T=1{,}3")]], notes: ["Trascrizione verificata sul render della pagina PDF 197; review umana cella per cella ancora obbligatoria."] },
    { id: t("6.5.i"), unit: "6.5.3.1.1", number: "6.5.I", page: 200, caption: "Coefficienti parziali γR per le verifiche agli stati limite ultimi di muri di sostegno", columnCount: 2, columnWidths: [58, 42], headers: [[cell("Verifica"), cell("Coefficiente parziale (R3)", "\\text{Coefficiente parziale }(\\mathrm{R3})")]], rows: [[cell("Capacità portante della fondazione"), cell("γR = 1,4", "\\gamma_R=1{,}4")], [cell("Scorrimento"), cell("γR = 1,1", "\\gamma_R=1{,}1")], [cell("Ribaltamento"), cell("γR = 1,15", "\\gamma_R=1{,}15")], [cell("Resistenza del terreno a valle"), cell("γR = 1,4", "\\gamma_R=1{,}4")]], notes: ["Trascrizione verificata sul render della pagina PDF 200; review umana cella per cella ancora obbligatoria."] },
    { id: t("6.6.i"), unit: "6.6.2", number: "6.6.I", page: 202, caption: "Coefficienti parziali per la resistenza degli ancoraggi", columnCount: 3, columnWidths: [33, 27, 40], headers: [[cell(""), cell("Simbolo"), cell("Coefficiente parziale")]], rows: [[cell("Temporanei"), cell("γR", "\\gamma_R"), cell("1,1")], [cell("Permanenti"), cell("γR", "\\gamma_R"), cell("1,2")]], notes: ["Trascrizione verificata sul render della pagina PDF 202; review umana cella per cella ancora obbligatoria."] },
    { id: t("6.6.ii"), unit: "6.6.2", number: "6.6.II", page: 202, caption: "Fattori di correlazione per derivare la resistenza caratteristica da prove di progetto, in funzione del numero degli ancoraggi di prova", columnCount: 4, columnWidths: [43, 19, 19, 19], headers: [[cell("Numero degli ancoraggi di prova"), cell("1"), cell("2"), cell("> 2", ">2")]], rows: [[cell("ξa1", "\\xi_{a1}"), cell("1,5"), cell("1,4"), cell("1,3")], [cell("ξa2", "\\xi_{a2}"), cell("1,5"), cell("1,3"), cell("1,2")]], notes: ["Trascrizione verificata sul render della pagina PDF 202; review umana cella per cella ancora obbligatoria."] },
    { id: t("6.6.iii"), unit: "6.6.2", number: "6.6.III", page: 203, caption: "Fattori di correlazione per derivare la resistenza caratteristica dalle prove geotecniche, in funzione del numero n di profili di indagine", columnCount: 6, columnWidths: [38, 12, 12, 12, 12, 14], headers: [[cell("Numero di profili di indagine"), cell("1"), cell("2"), cell("3"), cell("4"), cell("≥ 5", "\\ge5")]], rows: [[cell("ξa3", "\\xi_{a3}"), cell("1,80"), cell("1,75"), cell("1,70"), cell("1,65"), cell("1,60")], [cell("ξa4", "\\xi_{a4}"), cell("1,80"), cell("1,70"), cell("1,65"), cell("1,60"), cell("1,55")]], notes: ["Trascrizione verificata sul render della pagina PDF 203; review umana cella per cella ancora obbligatoria."] },
...
for (const table of tables as any[]) for (const row of [...table.headers, ...table.rows]) for (const tableCell of row) tableCell.align = "center";
...
function blockRecord(unit: UnitSpec, block: BlockSpec, index: number): any {
    ...
    return { blockId, kind: block.kind, origin: "official", ...(block.listMarker ? { listMarker: block.listMarker } : {}), ...(block.listLevel !== undefined ? { listLevel: block.listLevel } : {}), text: { raw: source, normalized, normalizationVersion: profile, ...(segments ? { inline: segments } : {}) }, evidence: evidence(block.page, source, normalized) };
}
