...
{
    const path = join(dir, "7.5.3.2.json");
    const unit = JSON.parse(await readFile(path, "utf8")) as { blocks: Array<{ blockId: string; kind: string; origin: string; assetId?: string; text?: { raw?: string; normalized: string; normalizationVersion?: string; inline?: Part[] }; evidence?: any }> };
    if (!unit.blocks.some((block) => block.text?.normalized === "La duttilità locale è definita come segue:")) {
        const formulaIndex = unit.blocks.findIndex((block) => block.assetId?.endsWith(":7.5.3.2:mu-local"));
        ok(formulaIndex >= 0, "Formula μ della duttilità locale mancante");
        const raw = "La duttilità locale è definita come segue:";
        unit.blocks.splice(formulaIndex, 0, {
            blockId: "urn:structural-codes:it:unit:ntc2018:7.5.3.2#block-editorial-006a",
            kind: "paragraph",
            origin: "official",
            text: { raw, normalized: raw, normalizationVersion: "ntc75-editorial-profile-0.1.0", inline: [{ kind: "text", value: raw }] },
            evidence: {
                ...
                transformations: [{ operation: "manual-correction", ruleVersion: "ntc75-format-audit-step4-0.1.0", note: "Ripristinato il capoverso introduttivo verificato nel PDF ufficiale prima della formula non numerata sulla duttilità locale." }],
                rawSha256: createHash("sha256").update(raw).digest("hex"),
                normalizedSha256: createHash("sha256").update(raw).digest("hex"),
            },
        });
...
{
    const path = join(dir, "7.5.4.2.json");
    const unit = JSON.parse(await readFile(path, "utf8")) as { blocks: Array<{ blockId: string; kind: string; origin: string; text?: { raw?: string; normalized: string; normalizationVersion?: string; inline?: Part[] }; evidence?: any }> };
    const source = unit.blocks.find((block) => block.text?.normalized.startsWith("dove γ_{Rd}"));
    if (source?.text && !unit.blocks.some((block) => block.text?.normalized.startsWith("Nella [7.5.11]"))) {
        const boundary = " Nella [7.5.11]";
        const normalizedIndex = source.text.normalized.indexOf(boundary);
        const rawIndex = (source.text.raw ?? "").indexOf("Nella [7.5.11]");
        ok(normalizedIndex > 0 && rawIndex > 0, "Capoverso dopo [7.5.11] non individuato");
        const firstNormalized = source.text.normalized.slice(0, normalizedIndex).trimEnd();
        const secondNormalized = source.text.normalized.slice(normalizedIndex + 1).trimStart();
        const firstRaw = (source.text.raw ?? "").slice(0, rawIndex).trimEnd();
        const secondRaw = (source.text.raw ?? "").slice(rawIndex).trimStart();
        source.text.normalized = firstNormalized;
        source.text.raw = firstRaw;
        source.text.inline = [
            { kind: "text", value: "dove " },
            { kind: "math", value: "γ_{Rd}", latex: "\\gamma_{Rd}" },
            { kind: "text", value: " è dato in Tab. 7.2.I, " },
            { kind: "math", value: "M_{C,pl,Rd}", latex: "M_{C,pl,Rd}" },
            { kind: "text", value: " è la capacità a flessione della colonna calcolata per i livelli di domanda a sforzo normale valutata nelle combinazioni sismiche delle azioni ed " },
            { kind: "math", value: "M_{b,pl,Rd}", latex: "M_{b,pl,Rd}" },
            { kind: "text", value: " è la capacità delle travi che convergono nel nodo trave-colonna." },
        ];
        const sourceEvidence = source.evidence ?? {};
        sourceEvidence.rawSha256 = createHash("sha256").update(firstRaw).digest("hex");
        sourceEvidence.normalizedSha256 = createHash("sha256").update(firstNormalized).digest("hex");
        source.evidence = sourceEvidence;
        const secondEvidence = JSON.parse(JSON.stringify(sourceEvidence));
        secondEvidence.rawSha256 = createHash("sha256").update(secondRaw).digest("hex");
        secondEvidence.normalizedSha256 = createHash("sha256").update(secondNormalized).digest("hex");
        secondEvidence.transformations = [...(secondEvidence.transformations ?? []), { operation: "manual-correction", ruleVersion: profile, note: "Separato il capoverso che inizia con Nella [7.5.11] sulla base dello stacco tipografico verificato nel PDF ufficiale." }];
        unit.blocks.splice(unit.blocks.indexOf(source) + 1, 0, {
            blockId: "urn:structural-codes:it:unit:ntc2018:7.5.4.2#block-editorial-018a",
            kind: "paragraph",
            origin: "official",
            text: { raw: secondRaw, normalized: secondNormalized, normalizationVersion: "ntc75-editorial-profile-0.1.0", inline: [{ kind: "text", value: secondNormalized }] },
...
await edit("7.5.5");
for(const n of ["7.5.3.1","7.5.4.1","7.5.4.2","7.5.6"])await edit(n,u=>{for(const b of u.blocks){if(b.kind!=="paragraph"||b.text?.inline?.[0]?.kind!=="math")continue;b.kind="list-item";b.listMarker="none"}});
