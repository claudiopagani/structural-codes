...
const scopedPages279To284 = process.argv.includes("--pages-279-284");
const scopedUnits279To284 = new Set([
  "C8.7.1.3.1.1", "C8.7.1.3.1.2", "C8.7.1.3.2", "C8.7.1.3.3", "C8.7.1.4",
  "C8.7.2", "C8.7.2.1", "C8.7.2.1.1", "C8.7.2.1.2", "C8.7.2.2",
  "C8.7.2.2.1", "C8.7.2.2.2", "C8.7.2.2.3",
]);
...
const mathMap: Record<string, string> = {
  "ρ_i=D_i/C_i": "\\rho_i=D_i/C_i", "ρ_i≥1": "\\rho_i\\ge1", "ρ_i": "\\rho_i", "D_i": "D_i", "C_i": "C_i", "q = 1,5": "q=1{,}5", "0,5": "0{,}5", "h=h_i+h_j": "h=h_i+h_j", "M-N": "M-N", "N=0": "N=0", "b = h/l": "b=h/l", "P/lt": "P/(lt)", "t(l+h)/2": "t(l+h)/2",
  "f̃_{v0d}": "\\widetilde{f}_{v0d}", "μ̃": "\\widetilde{\\mu}", "f_{bt}=0,1f_b": "f_{bt}=0{,}1f_b", "f_{ftd}": "f_{ftd}", "f_{ft}": "f_{ft}", "f_{btd}": "f_{btd}", "f_{td}": "f_{td}", "f_t": "f_t", "f_b": "f_b", "f_{v0d}": "f_{v0d}", "f_{v0}": "f_{v0}", "f_{v,lim}": "f_{v,lim}", "f_h": "f_h",
  "φ_i": "\\varphi_i", "φ_j": "\\varphi_j", "u_i": "u_i", "u_j": "u_j", "u_0": "u_0", "h_i": "h_i", "h_j": "h_j",
  "V_t": "V_t", "V_{t,lim}": "V_{t,lim}", "V_R": "V_R", "V_w": "V_w", "V_{bu}": "V_{bu}", "d_{max}": "d_{max}", "d_{cu}": "d_{cu}",
  "d_0": "d_0", "σ_0": "\\sigma_0", "σ_y": "\\sigma_y", "τ_0": "\\tau_0", "τ_{0d}": "\\tau_{0d}", "μ": "\\mu", "φ": "\\varphi", "ρ": "\\rho", "ω": "\\omega", "ν": "\\nu", "60%": "60\\%", "40%": "40\\%", "10%": "10\\%", "30°": "30^\\circ", "0,577": "0{,}577", "0,4": "0{,}4", "0,2": "0{,}2", "0,02": "0{,}02", "0,015": "0{,}015", "0,005": "0{,}005", "0,004": "0{,}004",
  "FC": "\\mathrm{FC}", "q": "q", "N": "N", "M": "M", "P": "P", "L": "L", "I": "I", "E": "E", "W": "W", "T": "T",
};
...
const inlinePattern = new RegExp(fixedMathTokens.concat(singleMathTokens.map(escapeRegExp)).join("|"), "gu");
...
function addBlock(blocks: Block[], normalized: string, kind: "heading" | "paragraph" | "list-item", ev: unknown, rawText = normalized) {
  ...
  const segments = kind === "heading" ? undefined : inline(normalized); if (segments) text.inline = segments;
...
function makeUnit(official: string, title: string, parent: string, ancestors: string[], position: number, build: (blocks: Block[]) => void, assets: string[] = []) {
  if (scopedPages279To284 && !scopedUnits279To284.has(official)) return;
...
makeUnit("C8.7.1.3.1.1", "Pareti murarie", "C8.7.1.3.1", ["C8", "C8.7", "C8.7.1", "C8.7.1.3", "C8.7.1.3.1"], 1, (blocks) => {
  ...
  addText(blocks, r(279, 44, 47), "Il criterio di resistenza per la verifica a taglio scorrimento, da valutare nelle sezioni soggette a momento massimo (solitamente le due sezioni di estremità del pannello), è quello indicato al § 7.8.2.2.2 delle NTC, utilizzando i valori di f_{v0} indicati in tabella 8.5.1, con la precisazione che la limitazione f_{v,lim} relativa alla rottura dei blocchi è fornita dalla seguente relazione, ricavata per blocchi di forma standard:"); addFormulaRef(blocks, r(279, 48, 49), "C8.7.1.14"); addText(blocks, r(279, 50, 51), "dove f_b è la resistenza a compressione del blocco, normalizzata. Lo spostamento ultimo a SLC è definito in corrispondenza di una rotazione della corda, calcolata come sopra per la sezione di estremità di verifica, pari a 0.005."); addProse(blocks, r(279, 52)); addProse(blocks, r(279, 53)); addProse(blocks, r(279, 54), "list-item"); addProse(blocks, r(279, 55), "list-item"); addProse(blocks, r(279, 56, 60));
  ...
  addText(blocks, r(280, 28), "- le murature a tessitura irregolare, con rottura per trazione diagonale governata dal parametro τ_0;", "list-item"); addText(blocks, r(280, 29, 31), "- le murature a tessitura regolare, per le quali la fessurazione può essere “a scaletta”, ossia con andamento diagonale attraverso i giunti di malta (governata dal parametro di resistenza f_{v0}, associato idealmente alla crisi dei giunti), oppure diagonale attraverso gli inerti della muratura (governata dal parametro di resistenza f_{v,lim}).", "list-item");
  ...
  addText(blocks, r(280, 39), "l è la lunghezza del pannello;"); setLastInlineTerms(blocks, [["l", "l"]]);
  addText(blocks, r(280, 40), "t è lo spessore del pannello;"); setLastInlineTerms(blocks, [["t", "t"]]);
  addText(blocks, r(280, 41, 43), "σ_0 è la tensione normale media, riferita all’area totale della sezione (= P/lt, con P forza assiale agente, positiva se di compressione);"); setLastInlineTerms(blocks, [["σ_0", "\\sigma_0"], ["P/lt", "P/(lt)"], ["P", "P"]]);
  ...
  addText(blocks, r(280, 50, 51), "b è un coefficiente correttivo legato alla distribuzione degli sforzi sulla sezione, dipendente dalla snellezza della parete. Si può assumere b = h/l, comunque non superiore a 1,5 e non inferiore a 1, dove h è l’altezza del pannello."); setLastInlineTerms(blocks, [["b = h/l", "b=h/l"], ["b", "b"], ["h", "h"]]);
...
makeUnit("C8.7.2.2.1", "Analisi statica lineare", "C8.7.2.2", ["C8", "C8.7", "C8.7.2", "C8.7.2.2"], 1, (blocks) => {
  addProse(blocks, r(284, 9, 10)); addText(blocks, r(284, 11, 14), "– considerando tutti gli elementi primari della struttura ed indicato, per l’i-esimo di tali elementi, con ρ_i=D_i/C_i il rapporto tra il momento flettente D_i fornito dall’analisi della struttura soggetta alla combinazione di carico sismica e il corrispondente momento resistente C_i (valutato in presenza dello sforzo normale relativo alle condizioni di carico gravitazionali), il coefficiente di variazione di tutti i ρ_i≥1 non deve superare il valore di 0,5;", "list-item"); addText(blocks, r(284, 15, 17), "– la capacità C_i degli elementi/meccanismi fragili è maggiore della corrispondente domanda D_i, quest’ultima calcolata sulla base della capacità degli elementi duttili adiacenti, se il ρ_i degli elementi/meccanismi fragili è maggiore di 1, oppure sulla base dei risultati dell’analisi, se il ρ_i degli elementi/meccanismi fragili è minore di 1.", "list-item");
  addText(blocks, r(284, 18), "Analisi statica lineare con spettro elastico", "heading"); addProse(blocks, r(284, 19, 20));
  addText(blocks, r(284, 21), "Analisi statica lineare con fattore di comportamento q", "heading"); addProse(blocks, r(284, 22, 27)); addProse(blocks, r(284, 28, 29)); addText(blocks, r(284, 30, 33), "La domanda sugli elementi strutturali si ottiene dall’analisi con spettro di risposta elastico ridotto, rispettivamente, per gli elementi/meccanismi “duttili” del fattore di comportamento attribuito alla struttura, per gli elementi/meccanismi “fragili” del fattore di comportamento q = 1,5. Per questi ultimi la domanda non può superare quella trasmessa dagli elementi/meccanismi duttili ad essi alternativi, valutata come indicato al punto b) del § C8.7.2.2.");
});
...
const formulas = [
  { id: F15, unitId: unitId("C8.7.1.3.1.1"), officialNumber: "C8.7.1.15", pdfPage: 280, latex: "f_{ftd}=\\min\\left(\\frac{f_{btd}}{2};f_{v0d}+\\frac{\\mu\\sigma_y}{\\Phi}\\right)" },
...
writeFileSync(join(ASSETS, "C8.7-step2a.json"), JSON.stringify({ $schema: "urn:structural-codes:schema:asset-manifest:v2", schemaVersion: "2.0.0-alpha.2", recordType: "asset-manifest", document: "circ2019", section: "C8.7-step2a", sourceId: SOURCE_ID, status: "draft", formulas, tables: [], figures: [] }, null, 2) + "\n", "utf8");
